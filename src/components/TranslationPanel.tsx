...
export default function TranslationPanel({ messages }: TranslationPanelProps) {
  ...
  return (
    <div className="h-full flex flex-col min-h-0 bg-gray-800/40 border-l border-gray-700/50 overflow-hidden">
      <div className="shrink-0 px-2.5 py-1.5 text-[9px] font-medium text-gray-500 uppercase tracking-wider border-b border-gray-700/30">
        English Translation
      </div>
      <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-2 py-2 space-y-1.5">
        {messages.map((msg) => (
          <div key={`trans-${msg.id}`} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div
              className={`max-w-[95%] px-2 py-1 leading-relaxed whitespace-pre-wrap break-words rounded ${
                msg.sender === 'user'
